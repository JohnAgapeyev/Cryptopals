...
int main(void) {
    ...
    unsigned char *decoded = hex_decode(testString, strlen(testString));
    unsigned char *buffer = base_64_encode(decoded, strlen(testString) / 2);

    if (strcmp(buffer, resultString) == 0) {
